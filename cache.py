...
class Cache:
    def __init__(self, cache_size, block_size, nway, address_length):
        ...
        self.offset_size_ = math.floor(math.log2(self.block_size_))
        self.index_size_ = math.floor(math.log2(self.set_count_))
    ...
    def create_cache(self):
        ...
        # cache entry = (valid, dirty, Set())
        for x in range(0, self.set_count_):
            self.cache_table_.append(0, 0, Set(x))
    ...
    # parameter address is a binary number of string type
        ...
        tag, index, offset = self.read_address(address)
        ...
        for x in self.cache_table_:
            if x.get_set_number() == index:
                result, data_item = x.read_block(tag, offset)
    ...
    def write_block(self, address, data):
        ...
        for x in self.cache_table_:
            if x.get_set_number() == index:
                result = x.write_block(tag, offset, data)
